...
export interface ForEachCallback {
    (node: Node, parentNode?: FolderNode, indexInFolder?: number): boolean
}

export interface PlainFolderNode {
    id?: number;
    name: string;
    sub?: Array<Node>
}

export interface FolderNode extends Node, CouldOpenNode, CouldRemoveNode,
...
export class FolderNode extends Node {
    ...
    name: string;
    sub: Array<Node | TempFileNode>;
    _isSubLoaded: boolean;
    _isSubInitialized: boolean;
    _isFolder: true;
    _type: FileType.Folder;

    constructor(fileSystem: FileSystem, {id, name, sub}: PlainFolderNode) {
        super(fileSystem);
        this.name = name;
        this.sub = sub || [];
    ...
    async loadSub() {
        ...
        if (files) {
            ...
            this.sub = folders.concat(fileProps.map(fps => {
                ...
                return (<TempFileNode>sub[index]).toFileNode(fps)
            }))
        }
     ...
     * @returns {Promise<void>}
     */
    async append(node: FileNode | FolderNode, shouldRename: boolean = true)
        : Promise<void> {
        const sub = this.sub;
        ...
        if (node instanceof FolderNode) {
            const index = sub.findIndex(n =>
                n instanceof FileNode || n instanceof TempFileNode);
