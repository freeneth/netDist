...
export class CouldCreateNode {
    applyToServer?: () => Promise<CouldCreateNode>;
    
    /**
     * 新建文件夹
     */
    async create(toFolder: FolderNode): Promise<this> {
    ...
    async createFolder(toFolder: FolderNode): Promise<this> {
        if (!(this instanceof FolderNode)) {
            throw new Error()
        }
        const node: FolderNode = this;
     ...
     */
    async createFile(toFolder: FolderNode): Promise<this> {
        if (!(this instanceof FileNode)) {
            throw new Error()
        }
        const node: FileNode = this;
        // 保存到 FileInfo
